    ...
    try {
      ...
      if (mode === 'tambah') {
        ...
        const response = await axios.put(
          `http://localhost:5000/api/laporan/${dataAwal._id}`,
          formData,
          config
        );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow-sm">
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(e) => setFormData({ ...formData, tingkatKekeringan: e.target.value })}
          >
      ...
      <Button
        ...
        className="w-full"
      >
        {loading ? 'Memproses...' : mode === 'tambah' ? 'Tambah Laporan' : 'Update Laporan'}
      </Button>
    </form>
  );
}
